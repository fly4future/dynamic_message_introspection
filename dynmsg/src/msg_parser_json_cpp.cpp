  ...
  uint8_t * buffer)
{
  ...
  *reinterpret_cast<CppType *>(buffer) = json.as<CppType>();
}

#ifdef DYNMSG_json_CPP_BAD_INT8_HANDLING
...
#endif  // DYNMSG_json_CPP_BAD_INT8_HANDLING
...
{
  DYNMSG_DEBUG(
    std::cout << "DEBUG: write_member_item<string>(): " << json.as<std::string>() << std::endl);
  using CppType =
    typename TypeMappingCpp<rosidl_typesupport_introspection_cpp::ROS_TYPE_STRING>::CppType;
  *reinterpret_cast<CppType *>(buffer) = json.as<std::string>();
}
...
{
  DYNMSG_DEBUG(
    std::cout << "DEBUG: write_member_item<wstring>(): " << json.as<std::string>() << std::endl);
  using CppType =
    typename TypeMappingCpp<rosidl_typesupport_introspection_cpp::ROS_TYPE_WSTRING>::CppType;
  *reinterpret_cast<CppType *>(buffer) = string_to_u16string(json.as<std::string>());
}
  ...
  uint8_t * buffer)
{
  ...
  auto seq = reinterpret_cast<SequenceType *>(buffer);
  seq->push_back(string_to_u16string(json.as<std::string>()));
}
  ...
  const MemberInfo_Cpp & member)
{
  ...
  DYNMSG_DEBUG(std::cout << json.size() << ":" << json << std::endl);
  ...
  const MemberInfo_Cpp & member)
{
  ...
  // Just cast the sequence to std::vector<bool> and copy json Value elements into it
  using SequenceType =
    typename TypeMappingCpp<rosidl_typesupport_introspection_cpp::ROS_TYPE_BOOL>::SequenceType;
  auto seq = reinterpret_cast<SequenceType *>(buffer);
  for (size_t i = 0; i < json.size(); i++) {
    seq->push_back(json[(int)i].as<bool>());
  }
}
...
// Convert a json Value into a field in the binary ROS message - generic
template<int RosTypeId>
void write_member(const Json::Value & json, uint8_t * buffer, const MemberInfo_Cpp & member)
{
  ...
  if (is_sequence(member)) {
    write_member_sequence<RosTypeId>(json[member.name_], buffer + member.offset_, member);
    return;
  }
  ...
  if (member.is_array_) {
    for (size_t i = 0; i < member.array_size_; i++) {
      write_member_item<RosTypeId>(
        json[member.name_][(int)i],
        buffer + member.offset_ + sizeof(CppType) * i
      );
    }
  } else {
    // Handle single-item members
    write_member_item<RosTypeId>(json[member.name_], buffer + member.offset_);
  }
}

// Convert a nested json sequence Value
...
{
  if (is_sequence(member)) {
    write_member_sequence_nested(json[member.name_], buffer + member.offset_, member);
    return;
  }
  ...
  const TypeInfo_Cpp * member_typeinfo =
    reinterpret_cast<const TypeInfo_Cpp *>(member.members_->data);
  if (member.is_array_) {
    for (size_t i = 0; i < json[member.name_].size(); i++) {
      json_to_rosmsg_impl(
        json[member.name_][(int)i],
    ...
    json_to_rosmsg_impl(json[member.name_], member_typeinfo, buffer + member.offset_);
  }
}

// Convert a json representation to a binary ROS message by looping over the expected fields of the
// ROS message and getting their values from the json
void json_to_rosmsg_impl(
  const Json::Value & root,
  const TypeInfo_Cpp * typeinfo,
  uint8_t * buffer)
{
  DYNMSG_DEBUG(std::cout << "DEBUG: json_to_rosmsg_impl" << std::endl);
  DYNMSG_DEBUG(
...
void json_and_typeinfo_to_rosmsg(
  ...
  void * ros_message)
{
  // Parse the json representation to an in-memory representation
  const Json::Value root(json_str);
  // Convert the json representation to a binary representation
  uint8_t * buffer = reinterpret_cast<uint8_t *>(ros_message);
  impl::json_to_rosmsg_impl(root, type_info, buffer);
}
  ...
  rcutils_allocator_t * allocator)
{
  ...
  json_and_typeinfo_to_rosmsg(type_info, json_str, reinterpret_cast<void *>(ros_msg.data));
  return ros_msg;
}
  ...
  const std::string & json_str)
{
  ...
  return json_and_typeinfo_to_rosmsg(type_info, json_str, allocator);
}
